module 0x6c949011b612d398bfe80a9307e3ade4ba61f6e6678451b4da5b35dad07c934a::artikasatkav105 {
    use std::signer;
    ...
    const E_INVALID_TOKEN_IDS: u64 = 13;
    const E_INVALID_LENGTH: u64 = 14;
    const E_INVALID_STARS: u64 = 15;
    ...
    const COLLECTION_NAME: vector<u8> = b"CarNFT Collection APTos";
    ...
    // Structure to store rarity and stars data
    struct RarityData has key {
        rarities: Table<u64, String>, // token_id -> RarityLabel
        stars: Table<u64, u64>,      // token_id -> Stars
    }
    ...
    struct MintedNFTs has key {
        ...
        token_owners: Table<address, u64>,
    }
    ...
    // Helper functions to determine attributes based on token_id
    ...
    // Initialize function to load rarity data
    public entry fun initialize(
        account: &signer,
        ipfs_link: String,
        total_cars: u64,
        token_ids: vector<u64>,
        rarity_labels: vector<String>,
        stars: vector<u64>
    ) acquires Config {
        let signer_address = signer::address_of(account);
        ...
        assert!(token_ids_len == TOTAL_NFTS, E_INVALID_LENGTH);
        assert!(vector::length(&rarity_labels) == TOTAL_NFTS, E_INVALID_LENGTH);
        assert!(vector::length(&stars) == TOTAL_NFTS, E_INVALID_LENGTH);

        // Check token_ids (must be 1 to 200)
        let i = 0;
        while (i < token_ids_len) {
            let token_id = *vector::borrow(&token_ids, i);
            assert!(token_id == (i + 1), E_INVALID_TOKEN_IDS);
            i = i + 1;
        };

        // Check stars (must be between 1 and 7)
        i = 0;
        while (i < token_ids_len) {
            let star = *vector::borrow(&stars, i);
            assert!(star >= 1 && star <= 7, E_INVALID_STARS);
            i = i + 1;
        };
        ...
        // Initialize RarityData with provided data
        if (!exists<RarityData>(CREATOR_ADDRESS)) {
            let rarities = table::new();
            let stars_table = table::new();
            let len = vector::length(&token_ids);
            let i = 0;
            while (i < len) {
                let token_id = *vector::borrow(&token_ids, i);
                let rarity = *vector::borrow(&rarity_labels, i);
                let star = *vector::borrow(&stars, i);
                table::add(&mut rarities, token_id, rarity);
                table::add(&mut stars_table, token_id, star);
                i = i + 1;
            };
            move_to(account, RarityData {
                rarities,
                stars: stars_table,
            });
        };
    ...
    // Function to update rarity data for flexibility
    public entry fun update_rarity_data(
        account: &signer,
        token_ids: vector<u64>,
        rarity_labels: vector<String>,
        stars: vector<u64>
    ) acquires Config, RarityData {
        let signer_address = signer::address_of(account);
        let config = borrow_global<Config>(CREATOR_ADDRESS);
        assert!(signer_address == config.admin_addr, E_INVALID_CREATOR);

        let rarity_data = borrow_global_mut<RarityData>(CREATOR_ADDRESS);

        // Check lengths
        let len = vector::length(&token_ids);
        assert!(len == vector::length(&rarity_labels), E_INVALID_LENGTH);
        assert!(len == vector::length(&stars), E_INVALID_LENGTH);
        ...
        while (i < len) {
            ...
            assert!(token_id > 0 && token_id <= TOTAL_NFTS, E_INVALID_TOKEN_IDS);
            assert!(star >= 1 && star <= 7, E_INVALID_STARS);

            table::upsert(&mut rarity_data.rarities, token_id, rarity);
            table::upsert(&mut rarity_data.stars, token_id, star);
            i = i + 1;
        };
    }

    // Function to update total_cars for voting flexibility
    public entry fun update_total_cars(account: &signer, new_total_cars: u64) acquires Config, Voting {
        let signer_address = signer::address_of(account);
        let config = borrow_global<Config>(CREATOR_ADDRESS);
        assert!(signer_address == config.admin_addr, E_INVALID_CREATOR);

        let voting = borrow_global_mut<Voting>(CREATOR_ADDRESS);
        let old_total_cars = voting.total_cars;
        voting.total_cars = new_total_cars;

        // Update car_votes table: add new indices if new_total_cars > old_total_cars
        let i = old_total_cars;
        while (i < new_total_cars) {
            if (!table::contains(&voting.car_votes, i)) {
                table::add(&mut voting.car_votes, i, 0);
            };
            i = i + 1;
        };
    }

    // Mint NFT with updated rarity logic
    #[randomness]
    entry fun mint_nft(minter: &signer) acquires MintedNFTs, CollectionConfig, CollectionOwnerObjConfig, RarityData {
        ...
        // Fetch rarity and stars from RarityData
        let rarity_data = borrow_global<RarityData>(CREATOR_ADDRESS);
        let rarity = *table::borrow(&rarity_data.rarities, token_id);
        let stars = *table::borrow(&rarity_data.stars, token_id);
        let stars_str = u64_to_string(stars);
        ...
        table::add(&mut minted_nfts.token_owners, minter_address, token_id);
    ...
    #[view]
    public fun get_stars(token_id: u64): u64 acquires RarityData {
        let rarity_data = borrow_global<RarityData>(CREATOR_ADDRESS);
        assert!(token_id > 0 && token_id <= TOTAL_NFTS, E_INVALID_CAR_INDEX);
        assert!(table::contains(&rarity_data.stars, token_id), E_NOT_INITIALIZED);
        *table::borrow(&rarity_data.stars, token_id)
    }

    #[view]
    public fun get_total_cars(): u64 acquires Voting {
        let voting = borrow_global<Voting>(CREATOR_ADDRESS);
        voting.total_cars
    }
